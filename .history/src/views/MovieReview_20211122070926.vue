...
export default {
  ...
  data: function() {
    ...
    return {
      index: index,
      reviewForm: {
        title: index !== undefined ? this.movieData.review_set[this.reviewIdx].title : "",
        content: index !== undefined ? this.movieData.review_set[this.reviewIdx].content : "",      
        rank: index !== undefined ? this.movieData.review_set[this.reviewIdx].rank : 0
      }
    }
  },
  methods: {
  ...
  computed: {
    ...
    reviewIdx() {
      for (let i=0; i<this.movieData.review_set.length; i++ ) {
        if (this.movieData.review_set[i].id === this.index) {
          return i
        }
      }
      return null
