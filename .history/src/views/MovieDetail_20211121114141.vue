<template>
  <div class="container-fluid">
    <div class="row">
      <div class="col-4">
        <img :src="posterPath" class="img-fluid" alt="">
      </div>
      <div class="col-8">
        <h2 class="text-start">{{ movieData.title }}</h2>
        <p class="text-start">{{ movieData.overview }}</p>
        <p>{{ movieData }}</p>
      </div> 
    </div>
    <div>
      <span>리뷰 목록</span>
      <button
        @click="getReviewForm"
      >
        리뷰 작성
      </button>
      <div class="list-group">
        <div 
          class="list-group-item list-group-item-action"
          v-for="review in reviews" 
          :key="review.id" 
        >
          <router-link :to="{ name: 'ReviewDetial', params: { movieID: movieData.id, reviewId: review.id }}"></router-link>
          {{ review.userName }} | {{ review.title }}
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { mapState } from 'vuex'

export default {
  name: 'MovieDetail',
  methods: {
    getReviewForm() {
      this.$router.push({ name: 'MovieReview', params: { movieId: this.movieData.id} })
    },
    reviewDetail() {
      this.$router.push({ name: 'MovieReview', params: { movieId: this.movieData.id, reviewID: this. } })
    }
  },
  computed: {
    ...mapState(['movieData']),
    reviews() {
      return this.movieData.review_set
    },
    posterPath() {
      return `https://image.tmdb.org/t/p/w500/${this.movieData.poster_path}`
    },
  },
}
</script>

<style>

</style>